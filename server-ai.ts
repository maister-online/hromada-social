...
async function main() {
  ...
  app.get('/api/health', (_req, res) => {
    ...
    res.status(200).json({ ok: true, status: 'ONLINE', gemini: Boolean(process.env.GEMINI_API_KEY), storage: 'firebase-client', firebaseConfiguredOnClient: 'VITE_FIREBASE_API_KEY/VITE_FIREBASE_PROJECT_ID/VITE_FIREBASE_STORAGE_BUCKET/VITE_FIREBASE_APP_ID', model: GEMINI_MODEL, persistentData: true, production: IS_PRODUCTION, port: PORT, timestamp: new Date().toISOString() });
  });
  ...
  if (!IS_PRODUCTION) { const { createServer: createViteServer } = await import('vite'); const vite = await createViteServer({ server: { middlewareMode: true }, appType: 'spa' }); app.use(vite.middlewares); }
  else { app.use(express.static(distPath, { index: 'index.html' })); app.use((_req, res) => res.sendFile(path.join(distPath, 'index.html'))); }
