...
if (startSearch !== -1 && startSynthesis !== -1) {
  const replacement = `async function ordinarySearch(query: string) {
  try {
...
if (startSynth !== -1 && startAnswer !== -1) {
  const replacement = `async function synthesizeSearchResults(query: string, sources: { title: string; url: string; snippet?: string }[], history: any[] = []) {
  const sourceText = sources.map((src, i) => (i + 1) + ". " + src.title + "\\n" + (src.snippet ? "Опис: " + src.snippet + "\\n" : "") + "URL: " + src.url).join("\\n\\n");
  const prompt = "__MASHUNYA_SYNTHESIS__\\nКористувач запитав: " + query + "\\n\\nЗнайдені вебджерела:\\n" + sourceText + "\\n\\nСформуй точну коротку відповідь українською. Використовуй лише інформацію з наведених результатів. Якщо джерела не дають однозначної відповіді — прямо скажи про це. Не вигадуй. Наприкінці додай «Джерела» з назвами джерел.";
