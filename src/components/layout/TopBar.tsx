...
export const TopBar: React.FC<TopBarProps> = ({ onSelectNavTab, activeNavTab }) => {
  ...
  const navMenuItems = [
    ...
    { id: 'business', label: 'Оголошення та бізнес', icon: Briefcase },
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 py-2.5 flex items-center gap-2 sm:gap-4">
        <div className="flex items-center gap-2 shrink-0 relative">
          ...
          {showLeftNavMenu && <div className="absolute top-full left-0 mt-3 w-80 p-3 rounded-3xl bg-[#080b12]/95 border border-white/10 backdrop-blur-2xl shadow-2xl z-50 animate-fadeIn">
            ...
            <div className="max-h-[70vh] overflow-y-auto space-y-1">{navMenuItems.map(item => { const Icon = item.icon; const active = activeNavTab === item.id; return <button key={item.id} onClick={() => { onSelectNavTab(item.id); setShowLeftNavMenu(false); }} className={`w-full px-3 py-2.5 rounded-2xl flex items-center justify-between text-left transition-all ${active ? 'bg-cyan-400/15 text-cyan-200 border border-cyan-400/30' : item.highlight ? 'bg-amber-400/10 text-amber-200 border border-amber-400/20' : 'text-slate-300 hover:bg-white/5 border border-transparent'}`}><span className="flex items-center gap-3"><span className="p-2 rounded-xl bg-black/20"><Icon className="w-4 h-4" /></span><span className="text-xs font-semibold">{item.label}</span></span>{item.badge && <span className="text-[8px] px-2 py-1 rounded-full bg-emerald-400/10 text-emerald-300 border border-emerald-400/20">{item.badge}</span>}</button>; })}</div>
        ...
        <div className="relative flex-1 max-w-2xl mx-auto">
          ...
          {searchFocused && <div className="absolute top-full left-0 right-0 mt-2 p-3 rounded-3xl bg-[#080b12]/95 border border-white/10 backdrop-blur-2xl shadow-2xl z-50 animate-fadeIn"><div className="text-[10px] uppercase tracking-widest text-slate-500 mb-2">Швидкий доступ</div><div className="grid grid-cols-2 sm:grid-cols-3 gap-2">{[["problems","Проблеми",AlertTriangle],["petitions","Петиції",FileSpreadsheet],["business","Оголошення",Briefcase],["appeals","Звернення",FileText],["map","Карта",Map],["chat","Машуня AI",Bot]].map(([id,label,Icon]: any) => <button key={id} onClick={() => { onSelectNavTab(id); setSearchFocused(false); }} className="p-2.5 rounded-2xl bg-white/[.04] hover:bg-white/[.08] border border-white/5 text-slate-300 flex items-center gap-2 text-xs"><Icon className="w-4 h-4 text-cyan-300" />{label}</button>)}</div></div>}
        </div>
