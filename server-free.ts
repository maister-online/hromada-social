...
async function answer(message: string, history: any[] = []) {
  try {
    ...
    console.warn(`Mashunya AI error provider=gemini model=${GEMINI_MODEL}:`, JSON.stringify(details));
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`Mashunya server running on ${PORT}; Gemini=${Boolean(process.env.GEMINI_API_KEY)} model=${GEMINI_MODEL}`);
  });
}
